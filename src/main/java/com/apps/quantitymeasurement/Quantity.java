...
* 1. Subtraction and division, along with improved error handling for incompatible units and division by zero scenarios.
...
* 3. The toString method has also been overridden to provide a clear string representation of the Quantity object.
* 
...
public class Quantity<U extends IMeasurable> {
	...
	public Quantity<U> add(Quantity<U> other) {
		...
		// Check if the units are of the same measurable type
	    if (!this.unit.getClass().equals(other.unit.getClass())) {
	        throw new IllegalArgumentException("Cannot add quantities of different types");
	    }
	...
	**/
	public Quantity<U> subtract(Quantity<U> other) {
		...
		double subOfBase = thisBase - otherBase;
		
		double finalSub = subOfBase / this.unit.getConversionFactor();
		
		return new Quantity<>(finalSub, this.unit);
	}
	...
	*/
	public Quantity<U> subtract (Quantity<U> other, U targetUnit) {
		Quantity<U> totalValue = this.subtract(other);
		double totalInTargetUnit = totalValue.convertTo(targetUnit);
		return new Quantity<U>(totalInTargetUnit, targetUnit);
	}
	...
	*/
	public double divide (Quantity<U> other) {
		...
		double divideOfBase = thisBase / otherBase;
